...
Nhóm đề xuất chọn **migrate-mongo** (Truyền thống) và tập lệnh Node.js kết hợp **Gemini + `@faker-js/faker`** (AI) áp dụng trên hệ thống Proshop-v2 với 4 lý do cốt lõi sau:
...
- **Sinh dữ liệu và che giấu (Data Masking) thông minh**: Nhóm áp dụng chiến thuật phân vai. Thư viện `@faker-js/faker` xử lý việc xáo trộn các trường định danh phẳng (PII như tên, email) với tốc độ cao. Đồng thời, Gemini API được tích hợp để sinh dữ liệu kiểm thử biên (Edge Cases) và che giấu các mảng dữ liệu lồng nhau phức tạp (Nested JSON), đảm bảo dữ liệu sinh ra có tính logic ngữ cảnh mà không làm vỡ cấu trúc mô hình.
- **Tạo vòng lặp kiểm thử toàn vẹn bằng AI (Invariants Testing Loop)**: Việc sử dụng Gemini không chỉ dừng ở che giấu dữ liệu, mà còn đóng vai trò sinh các ca kiểm thử biên (Edge cases) và cố tình tạo ra dữ liệu lỗi (như thiếu trường bắt buộc, tính sai tổng tiền). Để hoàn thiện kịch bản kiểm thử, nhóm xây dựng các tập lệnh Node.js quét tự động kết hợp. Chính dữ liệu "độc hại" do AI sinh ra này là môi trường hoàn hảo để đối chứng và kích hoạt hệ thống phát hiện lỗi logic hoặc dữ liệu mồ côi (Orphaned Data) — qua đó khắc phục triệt để điểm yếu thiếu Khóa ngoại (Foreign Keys) của cơ sở dữ liệu NoSQL.
