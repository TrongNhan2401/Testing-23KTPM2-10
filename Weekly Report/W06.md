...
| --------- | -------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| Gemini    | Revising and redesigning the in-class activity: modifying the activity flow, rules, and interaction mechanics             | Updated activity design and script          |
| Claude    | Editing and polishing slide content, creating the presentation script, and building the complete `test_invariants_skill.md` Agent Skill workflow | Slide content, presentation script, `test_invariants_skill.md` |
...
- **Used Gemini Chat** to revise the in-class activity: redesigning the activity flow and interaction mechanics to better fit the seminar objectives and classroom constraints.
- **Used Claude Chat** to finalize deliverables from the brainstorming phase: editing and polishing the seminar slides, creating the presentation script (speaker notes, transition cues, and timing markers), and most importantly building the complete Agent Skill workflow captured in `test_invariants_skill.md`, which encodes the standard procedure for generating NoSQL invariant tests (read real schema → place orphan-check in the referencing collection → run via `runAllInvariants.js` → self-checklist).
...
- All AI-generated content — activity revisions, slide content, presentation script, skill instructions, and test code — was reviewed, refined, and verified by group members before being adopted into the repository or the seminar materials.
